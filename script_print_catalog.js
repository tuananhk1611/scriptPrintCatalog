...
request.open('GET', 'https://api.shopbase.com/v1/pod/catalogs', true); request.onload = function () {
    ...
    if (request.status >= 200 && request.status < 400) {
        ...
        $('.s-option').on('click', function (e) {
            const filterValue = $(this).attr('value'); $('.base-products').each(function (i, obj) { if (filterValue == 'All' || $(this).attr('group-name') == filterValue) { $(this).css('display', 'block') } else { $(this).css('display', 'none') } })
...
$('body').on('click', '.button-close', () => { closeModal(); })
