...
request.open('GET', 'https://api.shopbase.com/v1/pod/catalogs', true); request.onload = function () {
    ...
    if (request.status >= 200 && request.status < 400) {
        ...
        var listContent = ''; var allBadge = []; var listUniqueBadge = []; var badgeOptions = ''; var badgeFilter = {}; var listId = []; var bestSelling = {}; if (data.result.list_badge.list !== null) {
            data.result.list_badge.list.forEach(function (item, index) {
                ...
                if (item.is_root != true) { if (listId.indexOf(badgeId) < 0) { listId.push(badgeId); listUniqueBadge.push(item); listUniqueBadge.sort((a, b) => a.name.localeCompare(b.name)); } }
                var key = item.base_product_id
                if (badgeFilter['prod-' + key] === undefined) { badgeFilter['prod-' + key] = [badgeId]; } else { badgeFilter['prod-' + key].push(badgeId); }
                if (allBadge['prod-' + key] === undefined) { allBadge['prod-' + key] = [item]; } else { allBadge['prod-' + key].push(item); }
                if (item.is_root == true) { bestSelling = item; }
        ...
        var container = document.querySelectorAll('.contetnt-printbase')[0]; injectData.map(function (item, indexCat) {
            listItem = ''
            item.list_base_product.map(function (product, indexProduct) {
                ...
                if (allBadge['prod-' + product.id]) {
                    allBadge['prod-' + product.id].forEach(function (item, index) {
                        ...
                        if (item.is_root !== false) { badge = "<img src='https://uploads-ssl.webflow.com/5ed4c2a86103a3136bf1e97c/6013b9c4283fef28a6eab6d5_Best%20selling%401x.svg' style='display:inline-block;width:48px;height:48px;position:absolute;top:7px;left:8px;' />" } else {
                ...
                listItem += `<div class="base-products" style="position:relative" data-value-id="${badgeFilter['prod-' + product.id]}" data-cat="${indexCat}" data-index="${indexProduct}" badge-id="${badgeId}" group-name="${product.group_name
                    ...
                    `<div class="detail-content" style="margin-bottom:12px;min-height:141px;">
                    <p class="title">
                    ...
                    <p class="title">
                        <strong class="bold">Artwork requirement</strong>
                    </p>
                    <p class="description">
            ...
            if (indexCat === 0) {
                cashBackFilter = `<div id="cashback_filter">
                        <div id="cashback-select-area" style="cursor: pointer;">
                        <div class="cashback-select-label" style="cursor: pointer;">
                        <label style="margin-bottom:0;font-weight: 500; font-size: 16px; color: rgba(36, 39, 46, 0.5);cursor: pointer;">Product Group: <span id="select-cashback-value" style="color:rgb(36, 39, 46)">All</span></label>
                        </div>
        ...
        $('.catalog').fadeIn()
        $('#cashback-select-area').on('click', function () { const open = $('#cashback_select_options').css('display'); $('#cashback_select_options').css("display", `${open == 'none' ? 'block' : 'none'}`) })
        $('#select-area').on('click', function () { const open = $('#select_options').css('display'); $('#select_options').css("display", `${open == 'none' ? 'block' : 'none'}`) })
        $('.s-cashback-option').on('click', function (e) {
            const filterValue = $(this).attr('value'); $('.s-cashback-option').removeAttr('data-select'); $(this).attr('data-select', 1); var badgeSelected = $('.s-option[data-select="1"]').attr('value'); $('.base-products').each(function (i, obj) {
                badgeIds = $(this).attr('data-value-id').split(',')
                if (badgeSelected == '0') { if (filterValue == 'All' || $(this).attr('group-name') == filterValue) { $(this).css('display', 'block') } else { $(this).css('display', 'none') } } else { if ((filterValue == 'All' || $(this).attr('group-name') == filterValue) && badgeIds.includes(badgeSelected)) { $(this).css('display', 'block') } else { $(this).css('display', 'none') } }
            })
            $('#select-cashback-value').text(filterValue)
            $('#cashback_select_options').css('display', 'none')
        })
        $(".tier-icon").hover(function () { $(this).parent().find('.tierToolTip').show(); }, function () { $(this).parent().find('.tierToolTip').hide(); })
        $('.s-option').on('click', function (e) {
            const filterValue = $(this).attr('value'); const filterValueName = $(this).attr('data-value-name'); var cashBackSelected = $('.s-cashback-option[data-select="1"]').attr('value'); $('.s-option').removeAttr('data-select'); $(this).attr('data-select', 1); $('.base-products').each(function (i, obj) {
                badgeIds = $(this).attr('data-value-id').split(',')
                const groupName = $(this).attr('group-name')
                if (cashBackSelected == 'All') { if ((filterValue == '0' || badgeIds.includes(filterValue))) { $(this).css('display', 'block') } else { $(this).css('display', 'none') } } else { if ((filterValue == '0' || badgeIds.includes(filterValue)) && cashBackSelected == groupName) { $(this).css('display', 'block') } else { $(this).css('display', 'none') } }
            })
            $('#select-value').text(filterValueName)
            $('#select_options').css('display', 'none')
        })
        var navHeight = 410
        $('.catalog .navi .link-block-8').each(function (index, el) { navHeight += $(el).outerHeight() })
        ...
        <tr class="first-tr">
            ...
            if (productDescriptionData.selling_guide !== null) {
                var sellingGuide = `<div id="sellingGuide" style="display:flex;flex-direction:row;">
                            ...
                            </div>`; productDescriptionPopup.insertAdjacentHTML('beforeend', sellingGuide)
            }
            if (productDescriptionData.selling_guide !== null) {
                var flagBlock = $('#sellingGuide').find('.selling-guide-content'); $.each(productDescriptionData.selling_guide, function (i, obj) {
                    switch (obj.language) {
                        case 'vi': flagBlock.append(`<a style='margin: 0 0 5px 5px;' target="_blank" href="${obj.link}">
                                        <img style="width:34px" src='https://www.shopbase.com/blog/wp-content/themes/boostFlow-blog/assets/img/Vietnam.png' />
                                    </a>`)
                            break; case 'zh': flagBlock.append(`<a style='margin: 0 0 5px 5px;' target="_blank" href="${obj.link}">
                                        <img style="width:34px" src='https://www.shopbase.com/blog/wp-content/themes/boostFlow-blog/assets/img/cn-flag.png' />
                                    </a>`)
                            break; case 'en': flagBlock.append(`<a style='margin: 0 0 5px 5px;' target="_blank" href="${obj.link}">
                                        <img style="width:34px" src='https://www.shopbase.com/blog/wp-content/themes/boostFlow-blog/assets/img/United-States.png' />
                                    </a>`)
                            break; break; default: flagBlock.append("<a style='margin: 0 0 5px 5px;background-color:#bcc2be;color:white;width:34px;height:24px;text-decoration:none;text-align:center;' target='_blank' href='" + obj.link + "'>" + obj.language + "</a>")
                    }
                })
            }
        ...
        if (hashNav && !hashProduct) { window.location.href = `#${hashNav}` }
    } else { console.log('error') }
}
$('body').on('click', '.button-close', () => {
    let [_, hashNav, hashProduct] = window.location.hash.split('#')
    if (hashNav && hashProduct) { window.history.pushState('', '/', window.location.pathname) }
})
String.prototype.toSlug = function () { return this.toString().toLowerCase().replace(/\s/g, '-').replace(/[^a-zA-Z0-9-]/g, '') }
request.send()